...
class schafkopf(game):
    ...
    # added Dez 04 2020
    def playGame(self, declarations, table_cards, start_cards):
        for player in self.players:
            player.hand = start_cards[player.name]

        if declarations[0] ==0: # 0 means not declared yet!
            return self.phase, "No Error"

        # do the declaration
        for i in range(declarations):
            if len(declarations[i])>0:
                if declarations[i] in self.getPossDeclarations(self.players[i].hand):
                    self.assignDeclaration([], i, decl=declarations[i], use_random=False)
                else:
                    return "error", "Player "+str(i)+" "+self.player_names[i]+" cannot declare: "+declarations[i]
    ...
    def getIdFromName(self, my_name):
    ...
    def getColoredCards(self, cards, color):
        if "geier" in self.matching["type"]:
            cards = self.getAnyCards(cards, colors=[color], values=["7", "8", "9", "U", "K", "X", "A"])
        elif "wenz" in self.matching["type"]:
            cards = self.getAnyCards(cards, colors=[color], values=["7", "8", "9", "O", "K", "X", "A"])
        else:
            cards = self.getAnyCards(cards, colors=[color], values=["7", "8", "9", "K", "X", "A"])
        return cards
